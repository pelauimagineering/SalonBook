...
class JFormFieldPermissions extends JFormField
{
	...
	protected function getInput()
	{
		...
		$left = '';
		$right = '';
		JHtml::_('stylesheet', 'com_salonbook/permissions.css', array(), true);
		JHtml::_('script', 'com_salonbook/permissions.js', true, true);
		...
		foreach ($groups as $group)
		{
			...
			foreach ($access as $label => $actions)
			{
				...
				$right .= '<legend>'.JText::_('COM_SALONBOOK_'.$label.'_FIELDSET_LABEL').'</legend>';
	...
	protected function getAssetId()
	{
		...
		$db = JFactory::getDbo();
		$db->setQuery('SELECT id FROM #__assets WHERE name = "com_salonbook"');
	...
	{
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_salonbook/access.xml');
